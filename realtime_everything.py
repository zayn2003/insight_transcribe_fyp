...
def run_llm(text_chunk: str) -> str:
    prompt = f"""
You are a helpful assistant with strong computer-science knowledge. For the text
below, do:
1. List key CS terms that appear.
2. Give a one-sentence definition for each term.
3. Provide a concise insight / takeaway.
4. If appropriate, add a 1-2 sentence summary.
...
async def flush_and_report():
    ...
    if full_transcript:
        raw_text = " ".join([e["text"] for e in full_transcript])[:2000]  # clip to reduce load
